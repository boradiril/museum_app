...
- [ ] **Candidate-to-LLM pipeline for `/api/curate` — staged approach, decided 2026-09-24.** Sending the full `met_objects` pool to Claude on every request doesn't scale even at current size: a compact per-object representation (title/artist/culture/medium/date/tags/department — not display-only fields like image URLs) runs ~60-80 tokens each, so the current 1,157-row pool alone is ~80-90k tokens per request — expensive, slow, and worse for reasoning quality than a focused shortlist (LLMs are more reliable ranking a relevant subset than searching a large undifferentiated one themselves). Rule of thumb: comfortably "send it all" territory is under ~150-200 objects (~10-15k tokens); above that, narrow first.
  - **Phase 4 (now):** a lightweight, non-embeddings pre-filter — keyword/tag matching between the user's selected interests (and department relevance) against `tags`/`title`/`culture`/`classification` — narrowing the pool down to roughly 100-300 relevant candidates before the LLM call. No new infrastructure (pgvector, embedding pipeline) needed for this.
  - **Later (not scoped yet):** graduate to real embeddings/categorization (map Met's raw tags into our own fixed interest-category taxonomy, indexed array/GIN column; or compute a vector embedding per object via Postgres's `pgvector`, Supabase-native, for fuzzier matching including the free-text personalization field) computed once, offline, at seed time — not per-request. Trigger for this isn't a specific row count (real-world target eventually: 50,000-1,000,000+ rows across all Met departments/other museums) — it's when keyword matching starts failing (too few/too many results for common interest combos, or can't handle the free-text field's fuzzier signal well), which will likely bite well before the 50k-1M scale. Retrofitting embeddings onto an already-large seeded pool is much more expensive than designing it in from the start, so revisit before scaling seeding meaningfully past the current demo pool, not after.
- [ ] `getCandidateMetObjects()` (`src/lib/met-objects.ts`) has no `ORDER BY` — calling it unfiltered returns whatever order Postgres happens to produce, not a balanced cross-department sample, and repeat calls with the same filters return identical results every time (no variety). When Phase 4 needs balanced/varied sampling: simplest fix is calling it once per department with a per-department limit and merging client-side (no code change needed, uses the existing `departmentIds` filter); add `ORDER BY random()` as a `MetObjectFilters` option if call-to-call variety is also wanted. Build when Phase 4 actually needs it, not now.
