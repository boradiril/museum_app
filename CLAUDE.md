...
- [ ] Scaling `met_objects` retrieval past a demo-sized pool (Phase 3 currently: 1,157 rows across 3 departments; real-world target eventually: 50,000–1,000,000+ across all Met departments/other museums). Naive SQL filtering on raw Met tags stays fast at any scale (Postgres handles millions of indexed rows in milliseconds), but doesn't narrow candidates down *well* enough to hand a small, LLM-appropriate shortlist to `/api/curate` — and the Claude call itself (not the DB query) will be the dominant latency cost regardless of pool size. Recommended direction: do the expensive semantic work once, offline, at seed time — either map Met's raw tags/classification into our own fixed interest-category taxonomy (indexed array/GIN column), or compute a vector embedding per object via Postgres's `pgvector` (Supabase-native) for fuzzier matching, including the free-text personalization field. At request time, retrieval becomes a fast indexed lookup (categorical filter or ANN vector search) against work already done, keeping retrieval cost roughly flat as the pool grows from thousands to millions. Don't build this now — retrofitting embeddings onto a large already-seeded pool is much more expensive than designing it into the seed pipeline from the start, so revisit before scaling seeding meaningfully past the current demo pool.
